...
formatter = ColoredFormatter("%(log_color)s[%(levelname)s]%(reset)s %(white)s%(message)s",
                             datefmt=None,
                             reset=True,
                             log_colors={
                                 'DEBUG': 'cyan',
...
def add_movie(title, year, imdbid, tmdbid, rootFolderPath, qualityProfileId):
    ...
    global ProfileId
    if tmdbid == None:
        log.error("Failed to get tmdbid for{} {}.".format(title, year))
        return
...
def get_imdbid(title, year):
    ...
    r = requests.get("https://www.omdbapi.com/?t={}&y={}&type=movie&apikey={}".format(title, year, omdbapi_key),
                     headers=headers)
...
def main():
    ...
    with open(sys.argv[1], encoding="utf8") as csvfile:
        ...
        for row in m:
            ...
            qualityProfileId = int(row['qualityProfileId'])
            try:
                add_movie(title, year, imdbid, tmdbid, rootFolderPath, qualityProfileId)
